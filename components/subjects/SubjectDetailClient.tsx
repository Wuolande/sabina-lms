...
export function SubjectDetailClient({
  ...
  const [subject] = React.useState<Subject>(initialSubject);
  const [tutors] = React.useState<TutorProfile[]>(initialTutors);
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-brand-900 via-brand-800 to-slate-900 p-8 sm:p-12 text-white shadow-elevation space-y-4">
        ...
        <p className="text-base text-brand-100 max-w-2xl leading-relaxed">
          {subject.description ||
            "Learn 1-on-1 with accredited tutors, prepare for exams, or gain practical conversational confidence."}
        </p>

        <div className="flex flex-wrap items-center gap-4 pt-4">
          <Link href={`/find-tutors?subject=${subject.slug || subject.id}`}>
            <Button
              variant="secondary"
              size="lg"
              className="font-extrabold bg-accent-400 hover:bg-accent-500 text-slate-950 cursor-pointer"
            >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link
            href={`/find-tutors?subject=${subject.slug || subject.id}`}
