...
export function ClassroomSidebar({
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-slate-900 border-l border-y border-slate-700/80 p-2 rounded-l-2xl text-slate-300 hover:text-white shadow-xl transition"
      >
  ...
  return (
    <>
      ...
      <div
        className="lg:hidden fixed inset-0 z-40 bg-slate-950/75 backdrop-blur-xs animate-in fade-in duration-150"
        onClick={onToggle}
      />

      {/* Sidebar Drawer Container (Overlay on Mobile, Docked on Desktop) */}
      <aside className="fixed lg:relative inset-y-0 right-0 z-50 w-[88vw] max-w-sm sm:w-88 border-l border-slate-800 bg-slate-900/98 flex flex-col shrink-0 text-white select-none shadow-2xl animate-in slide-in-from-right duration-200">
        {/* Header Tabs */}
        <div className="flex items-center justify-between border-b border-slate-800 px-3 pt-2">
          ...
          <button
            onClick={onToggle}
            title="Close Panel"
            className="p-1.5 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 ml-1 transition"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
