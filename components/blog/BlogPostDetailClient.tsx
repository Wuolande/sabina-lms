...
export function BlogPostDetailClient({
  ...
  const [post] = React.useState<BlogPost>(initialPost);
  const [relatedPosts] = React.useState<BlogPost[]>(initialRelatedPosts);
  ...
  const shareOnTwitter = () => {
    if (typeof window !== "undefined" && post) {
      ...
      const text = encodeURIComponent(`Reading "${post.title}" on Sabina Education:`);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {post.authorTitle || "Education Contributor"}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="h-9 w-9 rounded-xl border border-slate-200 hover:border-slate-400 hover:bg-slate-50 flex items-center justify-center text-slate-700 transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="h-9 w-9 rounded-xl border border-slate-200 hover:border-emerald-400 hover:bg-emerald-50 flex items-center justify-center text-slate-700 hover:text-emerald-600 transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="h-9 px-3 rounded-xl border border-slate-200 hover:border-slate-400 hover:bg-slate-50 flex items-center gap-1.5 text-slate-700 text-xs font-bold transition-colors cursor-pointer"
            >
              {copied ? (
            ...
            {post.content.split("\n\n").map((block, idx) => {
              ...
              if (trimmed.startsWith("## ")) {
                return (
                  <h2
                    key={idx}
                    className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight pt-6 pb-2 border-b border-slate-100 font-heading"
                  >
                    {trimmed.replace("## ", "")}
                  </h2>
                );
              ...
              if (trimmed.startsWith("### ")) {
                return (
                  <h3
                    key={idx}
                    className="text-xl font-bold text-slate-900 tracking-tight pt-4 font-heading"
                  >
                    {trimmed.replace("### ", "")}
                  </h3>
                );
        ...
        <div className="mt-10 p-6 sm:p-8 rounded-3xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="space-y-1 flex-1">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              {post.authorBio ||
                `${post.author} writes on language pedagogy, exam preparation, and effective tutoring methodologies on Sabina.`}
            </p>
            {post.authorTwitter && (
        ...
        <div className="mt-12 rounded-3xl bg-slate-900 bg-gradient-to-br from-indigo-950 via-slate-900 to-[#14209C] p-8 sm:p-10 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4 max-w-xl">
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Book a personalized trial lesson on Sabina. Thousands of certified subject mentors available in your timezone.
            </p>
            <div className="pt-2">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Continue exploring insightful learning guides
                </p>
              </div>
              <Link href="/blog">
                <Button
