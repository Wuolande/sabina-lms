...
export interface FindTutorsClientProps {
...
export function FindTutorsClient({
  initialTutors = [],
  initialTotal = 0,
  initialSubjects = [],
  initialLanguages = [],
  initialCountries = [],
}: FindTutorsClientProps) {
  const searchParams = useSearchParams();

  const [tutors, setTutors] = React.useState<TutorProfile[]>(initialTutors);
  const [totalCount, setTotalCount] = React.useState(initialTotal || initialTutors.length);
  const [subjects, setSubjects] = React.useState<Subject[]>(initialSubjects);
  const [languages, setLanguages] = React.useState<Language[]>(initialLanguages);
  const [countriesList, setCountriesList] = React.useState<any[]>(initialCountries);
  const [isLoading, setIsLoading] = React.useState(!initialTutors || initialTutors.length === 0);

  // Session Type Tab: "all" | "private" | "group"
  const [sessionType, setSessionType] = React.useState<"all" | "private" | "group">("all");

  // Primary 4 Filter Card States
  ...
  // Load Reference Subjects, Languages & Countries if not passed
  React.useEffect(() => {
    if (subjects.length === 0) tutorService.getAllSubjects().then(setSubjects);
    if (languages.length === 0) tutorService.getAllLanguages().then(setLanguages);
    if (countriesList.length === 0) tutorService.getAllCountries().then(setCountriesList);
  }, [subjects.length, languages.length, countriesList.length]);
  ...
  React.useEffect(() => {
    if (isInitialMount.current) {
      ...
      if (initialTutors.length > 0 && !hasActiveFilters && page === 1 && sortBy === "popularity") {
        return;
      }
    }
    fetchTutors();
  }, [fetchTutors, initialTutors.length, hasActiveFilters, page, sortBy]);
                  ...
                  <SearchableSelect
                    ...
                    options={[
                      ...
                      ...countriesList.map((c: any) => ({
                        value: c.name || c,
                        label: c.name || c,
                        sublabel: c.continent ? `${c.continent} • ${c.code}` : undefined,
                      })),
                    ]}
                  />
                </div>
