...
export class TrainingRepository {
  ...
  async getCourseBySlug(slug: string, tutorId?: string): Promise<TrainingCourse | null> {
    const mock = mockTrainingCourses.find((c) => c.slug === slug);
    ...
    try {
      ...
      const modules: TrainingModule[] = (course.modules || mock?.modules || []).map((m: any) => ({
        id: m.id,
        ...
        resources: m.resources || [],
        isCompleted: completedModules.includes(m.id) || !!cert || (mock?.modules?.find(mm => mm.id === m.id)?.isCompleted)
      }));

      const quizData = course.quiz || mock?.quiz;
      const quiz: TrainingQuiz | undefined = quizData ? {
        ...
        questions: (quizData.questions || mock?.quiz?.questions || []).map((q: any) => ({
          id: q.id,
      ...
      return {
        ...
        modules,
        quiz,
        isEnrolled: !!enr || mock?.isEnrolled,
        progressPercentage: enr?.progress_percentage ?? mock?.progressPercentage ?? 0,
        status: cert ? 'completed' : enr?.status || mock?.status || 'not_started',
        completedAt: enr?.completed_at || cert?.issued_at || mock?.completedAt,
        certificateCode: cert?.certificate_code || mock?.certificateCode
      };
    } catch {
      return mock || null;
    }
  ...
  async submitQuiz(tutorId: string, quizId: string, courseId: string, answers: Record<string, number>): Promise<QuizSubmissionResult> {
    // Find quiz questions
    const course = mockTrainingCourses.find((c) => c.quiz?.id === quizId || c.id === courseId);
    const questions = course?.quiz?.questions || [];
    
    let correctCount = 0;
    const explanationList = questions.map((q) => {
    ...
    const passingScore = course?.quiz?.passingScore || 80;
    ...
    if (passed) {
      ...
      badgeTitle = course?.badgeTitle || 'Sabina Certified Educator';
      ...
      try {
        await Promise.all([
          adminSupabase.from('tutor_certificates').upsert({
            ...
            badge_icon: course?.badgeIcon || 'Award',
  ...
  async getCertificateById(certificateIdOrCode: string): Promise<TutorCertificate | null> {
    const mock = mockTutorCertificates.find(
      (c) => c.id === certificateIdOrCode || c.certificateCode === certificateIdOrCode
    );
